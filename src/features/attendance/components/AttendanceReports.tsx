...
export function AttendanceReports() {
      ...
      const reports = await Promise.all(
        dates.map(async (dateInterval) => {
          try {
            ...
            console.error(
              'Failed to fetch report for date:',
              dateInterval,
              error
            )
            return null
  ...
  const processDepartmentStats = (reports: AttendanceReport[]) => {
    ...
    const stats: DepartmentStats[] = Array.from(departmentMap.entries()).map(
      ([dept, data]) => ({
        department: dept,
  ...
  const exportReport = async () => {
    try {
      ...
      a.setAttribute(
        'download',
        `attendance_report_${format(dateRange.from, 'yyyy-MM-dd')}_to_${format(dateRange.to, 'yyyy-MM-dd')}.csv`
      )
  ...
  const averageAttendanceRate =
    reportData.length > 0
      ? reportData.reduce((sum, report) => sum + report.attendanceRate, 0) /
        reportData.length
      : 0
  const bestDay = reportData.reduce(
    (best, current) =>
      current.attendanceRate > (best?.attendanceRate || 0) ? current : best,
    reportData[0]
  )
  const worstDay = reportData.reduce(
    (worst, current) =>
      current.attendanceRate < (worst?.attendanceRate || 100) ? current : worst,
    reportData[0]
  )
